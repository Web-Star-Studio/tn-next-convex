...
function extractArgsFromSnippet(code) {
...
async function collectConvexFunctions(root) {
  ...
  for (const file of files) {
    ...
    while ((match = re.exec(code)) !== null) {
      ...
      const tail = code.slice(match.index, match.index + 2000);
      const args = extractArgsFromSnippet(tail);
      results.push({ name, kind, file: toPosix(path.relative(root, file)), args });
    }
  }
  return results.sort((a, b) => a.file.localeCompare(b.file) || a.name.localeCompare(b.name));
}

async function collectComponents(root) {
...
async function collectHooks(root) {
...
function inferNextRouteFromFile(file) {
...
async function collectNextApiRoutes(root) {
...
async function collectConvexHttpRoutes(root) {
...
function makeConvexUsageExample(fn) {
...
function makeNextApiExample(route) {
...
function makeConvexHttpExample(route) {
...
${r.file}`,
  ...
  const convexFnsMdParts = [header("Convex Functions", 2), ""];
  ...
  const componentsMdParts = [header("React Components", 2), "", "Import using your project's alias paths or relative imports.", ""];
  ...
  const hooksMdParts = [header("Custom Hooks", 2), ""];
